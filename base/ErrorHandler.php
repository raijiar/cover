...
abstract class ErrorHandler extends Component
{
    ...
    protected function handleFallbackExceptionMessage($exception, $previousException)
    {
        ...
        if (YII_DEBUG) {
            if (PHP_SAPI === 'cli') {
                ...
                echo '<pre>' . htmlspecialchars($msg, ENT_QUOTES, Cover::$app->charset) . '</pre>';
            }
        } else {
    ...
    public function handleError($code, $message, $file, $line)
    {
        if (error_reporting() & $code) {
            ...
            if (!class_exists('cover\\base\\ErrorException', false)) {
     ...
     * @since 1.0 this method is now public.
     */
    public function logException($exception)
    {
        $category = get_class($exception);
        if ($exception instanceof HttpException) {
            $category = 'cover\\web\\HttpException:' . $exception->statusCode;
        } elseif ($exception instanceof \ErrorException) {
            $category .= ':' . $exception->getSeverity();
        }
        Cover::error($exception, $category);
    }
