...
class Theme extends Component
{
    ...
    {
        $this->_baseUrl = $url === null ? null : rtrim(Cover::getAlias($url), '/');
    }
    ...
    {
        $this->_basePath = Cover::getAlias($path);
    }
    ...
    public function applyTo($path)
    {
        ...
        if (empty($pathMap)) {
            ...
            $pathMap = [Cover::$app->getBasePath() => [$basePath]];
        }
        $path = FileHelper::normalizePath($path);
        foreach ($pathMap as $from => $tos) {
            $from = FileHelper::normalizePath(Cover::getAlias($from)) . DIRECTORY_SEPARATOR;
            if (strpos($path, $from) === 0) {
                $n = strlen($from);
                foreach ((array) $tos as $to) {
                    $to = FileHelper::normalizePath(Cover::getAlias($to)) . DIRECTORY_SEPARATOR;
