...
class Security extends Component
{
     ...
     *
     * > Note: Cover's encryption protocol uses the same size for cipher key, HMAC signature key and key
     * derivation salt.
     */
     ...
     * @deprecated since version 1.0, [[generatePasswordHash()]] ignores [[passwordHashStrategy]] and
    ...
    protected function encrypt($data, $passwordBased, $secret, $info)
    {
        ...
        if ($encrypted === false) {
            throw new \cover\base\Exception('OpenSSL failure on encryption: ' . openssl_error_string());
        }
    ...
    protected function decrypt($data, $passwordBased, $secret, $info)
    {
        ...
        if ($decrypted === false) {
            throw new \cover\base\Exception('OpenSSL failure on decryption: ' . openssl_error_string());
        }
     ...
     * $hash = Cover::$app->getSecurity()->generatePasswordHash($password);
     * // ...save $hash in database...
     *
     * // during login, validate if the password entered is correct using $hash fetched from database
     * if (Cover::$app->getSecurity()->validatePassword($password, $hash)) {
     *     // password is good
     * } else {
     *     // password is bad
     * }
