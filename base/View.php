...
use cover\widgets\Block;
use cover\widgets\ContentDecorator;
use cover\widgets\FragmentCache;
...
class View extends Component implements DynamicContentAwareInterface
{
     ...
     * ```php
     * [
     *     'tpl' => ['class' => 'cover\smarty\ViewRenderer'],
     *     'twig' => ['class' => 'cover\twig\ViewRenderer'],
     * ]
     * ```
     *
     ...
     * @internal
     * @deprecated Since 1.0. Do not use this property directly. Use methods [[getDynamicContents()]],
     ...
     * @internal
     * @deprecated Since 1.0. Do not use this property directly. Use methods [[getDynamicPlaceholders()]],
    ...
    public function init()
    {
        ...
        if (is_array($this->theme)) {
            if (!isset($this->theme['class'])) {
                $this->theme['class'] = 'cover\base\Theme';
            }
            $this->theme = Cover::createObject($this->theme);
    ...
    protected function findViewFile($view, $context = null)
    {
        if (strncmp($view, '@', 1) === 0) {
            // e.g. "@app/views/main"
            $file = Cover::getAlias($view);
        } elseif (strncmp($view, '//', 2) === 0) {
            // e.g. "//layouts/main"
            $file = Cover::$app->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
        } elseif (strncmp($view, '/', 1) === 0) {
            // e.g. "/site/index"
            if (Cover::$app->controller !== null) {
                $file = Cover::$app->controller->module->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
            } else {
    ...
    {
        $viewFile = Cover::getAlias($viewFile);
        ...
        if ($this->beforeRender($viewFile, $params)) {
            Cover::debug("Rendering view file: $viewFile", __METHOD__);
            $ext = pathinfo($viewFile, PATHINFO_EXTENSION);
            if (isset($this->renderers[$ext])) {
                if (is_array($this->renderers[$ext]) || is_string($this->renderers[$ext])) {
                    $this->renderers[$ext] = Cover::createObject($this->renderers[$ext]);
                }
